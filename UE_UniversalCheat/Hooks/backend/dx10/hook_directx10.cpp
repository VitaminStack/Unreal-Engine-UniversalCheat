...
static bool CreateDeviceD3D10(HWND hWnd) {
    ...
    if (hr != S_OK) {
        LOG_ERROR("[!] D3D10CreateDeviceAndSwapChain() failed. [rv: %lu]\n", hr);
        return false;
    }
...
namespace DX10 {
    void Hook(HWND hwnd) {
        if (!CreateDeviceD3D10(GetConsoleWindow( ))) {
            LOG_ERROR("[!] CreateDeviceD3D10() failed.\n");
            return;
        }

        LOG_INFO("[+] DirectX10: g_pd3dDevice: 0x%p\n", g_pd3dDevice);
        LOG_INFO("[+] DirectX10: g_pSwapChain: 0x%p\n", g_pSwapChain);
