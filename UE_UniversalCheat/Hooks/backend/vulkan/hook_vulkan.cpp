...
namespace VK {
    void Hook(HWND hwnd) {
        ...
        PTR_VALIDATE(fnAcquireNextImageKHR);
        PTR_CHECK(fnAcquireNextImage2KHR);
        PTR_VALIDATE(fnAcquireNextImage2KHR);
        PTR_CHECK(fnQueuePresentKHR);
        PTR_VALIDATE(fnQueuePresentKHR);
        PTR_CHECK(fnCreateSwapchainKHR);
        PTR_VALIDATE(fnCreateSwapchainKHR);
        ...
        if (fnAcquireNextImageKHR) {
            ...
            MH_CHECK(MH_EnableHook(fnQueuePresentKHR));
            MH_CHECK(MH_EnableHook(fnCreateSwapchainKHR));
        }
    }
...
#include <Windows.h>
namespace VK {
    void Hook(HWND hwnd) { LOG_INFO("[!] Vulkan backend is not enabled!\n"); }
    void Unhook( ) { }
} // namespace VK
#endif
