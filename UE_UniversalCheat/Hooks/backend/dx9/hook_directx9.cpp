...
static bool CreateDeviceD3D9(HWND hWnd) {
    ...
    if (g_pD3D == NULL) {
        LOG_ERROR("[!] Direct3DCreate9() is failed.\n");
        return false;
    }
    ...
    if (hr != D3D_OK) {
        LOG_INFO("[!] CreateDevice() failed. [rv: %lu]\n", hr);
        return false;
    }
...
namespace DX9 {
    void Hook(HWND hwnd) {
        if (!CreateDeviceD3D9(GetConsoleWindow( ))) {
            LOG_INFO("[!] CreateDeviceD3D9() failed.\n");
            return;
        }

        LOG_INFO("[+] DirectX9: g_pD3D: 0x%p\n", g_pD3D);
        LOG_INFO("[+] DirectX9: g_pd3dDevice: 0x%p\n", g_pd3dDevice);
