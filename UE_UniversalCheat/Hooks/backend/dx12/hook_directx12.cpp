...
namespace DX12 {
    void Hook(HWND hwnd) {
        if (!CreateDeviceD3D12(GetConsoleWindow( ))) {
            LOG_ERROR("[!] CreateDeviceD3D12() failed.\n");
            return;
        }

        LOG_INFO("[+] DirectX12: g_pd3dDevice: 0x%p\n", g_pd3dDevice);
        LOG_INFO("[+] DirectX12: g_dxgiFactory: 0x%p\n", g_dxgiFactory);
        LOG_INFO("[+] DirectX12: g_pd3dCommandQueue: 0x%p\n", g_pd3dCommandQueue);
        LOG_INFO("[+] DirectX12: g_pSwapChain: 0x%p\n", g_pSwapChain);
        ...
        if (g_pd3dDevice) {
            ...
            MH_CHECK(MH_CreateHook(reinterpret_cast<void**>(fnExecuteCommandLists), &hkExecuteCommandLists, reinterpret_cast<void**>(&oExecuteCommandLists)));
            ...
            MH_CHECK(MH_EnableHook(fnExecuteCommandLists));
        }
    }
...
#include <Windows.h>
namespace DX12 {
    void Hook(HWND hwnd) { LOG_INFO("[!] DirectX12 backend is not enabled!\n"); }
    void Unhook( ) { }
} // namespace DX12
#endif
